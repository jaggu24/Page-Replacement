...
int find_min_index(int *array,int capacity)
{
...
int page_replacement(int n,int capacity,int* pages)                //MAIN LEAST FREQUENTY USED ALGORITHM
{
    ...
    for(i=0;i<n;i++)                                            //RUNNING THE LOOP FOR ALL PAGES REFERED
    {
        ...
        else                                                                //IF MAINMEMORY IS FULL
        {
            ...
            {
                int index=find_min_index(array->fqbit,capacity);               //FIND THE INDEX WHICH CONTAINS MINIMUM FREQUENCY
                int minvalue=array->fqbit[index];                              //STORES THE FREQUENCY
                int freq=frequency_item(array->fqbit,minvalue,capacity);       //FIND NO OF FRAMES HAVING SAME MINIMUM FREQUENCY VALUE
                pagefault++;
                if(freq==1)                                                     //IF ONLY ONE PAGE HAVING MINIMUM FREQUENCY
                {
                ...
                else                                                            //IF MORE THAN ONE FRAME HAVING SAME MINIMUM FREQUENCY
                {
                    int same[freq];                                             //CREATING AN ARRAY OF SIZE OF FREQ(NO OF PAGE HAVING SAME MIN FREQ)
                    int j,poi=0;
                    for(j=0;j<capacity;j++)                                     //FINDING THE INDEX OF FRAME WHICH HAVING SAME MINIMUM FREQUENCY
                    {
                        if(poi==freq)
                            break;
                        if(array->fqbit[j]==minvalue)
                        {
                            same[poi]=j;
                            poi++;
                        }
                    ...
                    poi=99;
                    for(j=0;j<freq;j++)                                       //FOR EACH SAME MIN FREQ CHECK WHICH FRAME COMES FIRST THAT FRAME IS
                    {                                                         //            REPLACED
